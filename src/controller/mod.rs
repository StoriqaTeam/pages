...
impl Controller for ControllerImpl {
    ...
    fn call(&self, req: Request) -> ControllerFuture {
        ...
        match (req.method().clone(), Route::from_path(&path)) {
            (Get, Some(Route::Page { identifier })) => {
                serialize_future(service.get_page(identifier))
            }
            (Post, Some(Route::Pages)) => serialize_future(
                parse_body::<NewPage>(req.body())
                    .map_err(|e| {
                        e.context("Failed to parse NewPage body")
                            .context(Error::Parse)
                            .into()
                    })
            ...
            (m, _) => Box::new(future::err(
                format_err!(
                    "Request to invalid endpoint in microservice! {:?} {:?}",
                    m,
                    path
